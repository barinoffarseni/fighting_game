...
class Fighter extends SpriteAnimated {
  ...
  render() {
    ...
    if (debug) {
      ...
      if (
        this.state == "attack1" ||
        (this.state == "attack2" && this.currentFrame == this.attackFrame)
      ) {
        ctx.fillStyle = "yellow"
      }

      ctx.fillRect(
        this.getAttackBoxPosition().x,
        this.getAttackBoxPosition().y,
        this.atackBox.width * this.direction,
  ...
  setState() {
    ...
    if (
      (this.state != this.newState && this.stateCanBeChanged) ||
      this.restartState
    ) {
  ...
  setAttackBoxMinMaxPosition() {
    if (this.direction > 0) {
      ...
      this.attackBoxXMax =
        this.getAttackBoxPosition().x + this.atackBox.width * this.direction
    } else {
      this.attackBoxXMin =
        this.getAttackBoxPosition().x + this.atackBox.width * this.direction
  ...
  render() {
    if (gameOver) {
      ...
      ctx.fillText(
        ...
        this.text.position.y + this.text.offset.y,
      )
    }
  }
}
...
class HealthBar extends Indicator {
  ...
  render() {
    ...
    ctx.fillRect(
      this.position.x + this.offset.x,
      this.position.y + this.offset.y,
      this.maxWidth * this.direction * this.healthValue,
      this.height,
    )
  }
  ...
  update() {
    if (gameOver) {
      canvas.addEventListener("click", function (event) {
        ...
        if (
          mouseX > restartButton.minX &&
          mouseX < restartButton.maxX &&
          mouseY > restartButton.minY &&
          mouseY < restartButton.maxY
        ) {
          location.reload()
        }
      })
